...
class RandomModelGeneratorTestCase(unittest.TestCase):
    def test_init(self):
        random_model_generator_1 = core.RandomModelGenerator()
        self.assertEqual(random_model_generator_1.options, {'id':None, 'name':None, 'version':None})

        random_model_generator_2 = core.RandomModelGenerator(options={'id':'test_rand', 'name':'test random model', 'version':'0.0'})
        self.assertEqual(random_model_generator_2.options, {'id':'test_rand', 'name':'test random model', 'version':'0.0'})

    def test_run(self):
        random_model_1 = core.RandomModelGenerator().run()
        self.assertIsNone(random_model_1.id)
        self.assertIsNone(random_model_1.name)
        self.assertIsNone(random_model_1.version)

        random_model_2 = core.RandomModelGenerator(options={'id':'test_rand', 'name':'test random model', 'version':'0.0'}).run()
