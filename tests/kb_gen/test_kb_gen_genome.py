...
class TestGenomeGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.seq = cell.species_types[0].get_seq()
        ...
        for i_gene, gene in enumerate(genes):
            self.gene_seqs.append(gene.get_seq())
            if i_gene == len(genes) - 1:
                self.intergene_seqs.append(seq_str[genes[i_gene].end:])
            else:
                self.intergene_seqs.append(seq_str[genes[i_gene].end:genes[i_gene+1].start-1])
    ...
    def test_number_of_genes(self):
        self.assertEqual(len(self.gene_seqs), GEN_NUM)
        self.assertEqual(len(self.intergene_seqs), GEN_NUM)
