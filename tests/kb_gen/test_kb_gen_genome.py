...
class TestGenomeGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.gen.clean_and_validate_options()
    ...
    def test_num_chromosomes(self):
        chromosomes = self.gen.knowledge_base.cell.species_types.get(__type=wc_kb.core.DnaSpeciesType)
    ...
    def test_rna_num(self):
        rnas = self.gen.knowledge_base.cell.species_types.get(__type=wc_kb.core.RnaSpeciesType)
        tus = self.gen.knowledge_base.cell.loci.get(__type=wc_kb.core.TranscriptionUnitLocus)
        self.assertEqual(len(rnas), len(tus))
        
        

    def test_prot_num(self):
        prots = self.gen.knowledge_base.cell.species_types.get(__type=wc_kb.core.ProteinSpeciesType)
        genes = self.gen.knowledge_base.cell.loci.get(__type=wc_kb.core.GeneLocus)
        geneCount = 0
        for gene in genes:
    ...
    def test_utrs (self): #checks average lengths of 5'/3' UTRs on transcription units with mRna
        ...
        self.assertAlmostEqual(sum_five_prime/mRnaCount, five_prime_len, 3 * math.sqrt(five_prime_len))
        self.assertAlmostEqual(sum_three_prime/mRnaCount, three_prime_len, 3 * math.sqrt(three_prime_len))
    ...
    def test_operons(self):
        ...
        self.assertAlmostEqual(avg_in_operon, operon_prop, 3 * math.sqrt(operon_prop))

        self.assertAlmostEqual(avg_operon_gen, operon_gen_num, 3 * math.sqrt(operon_gen_num))
